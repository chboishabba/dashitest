...
- `scripts/news_slice.py` fetches headlines/events for a time window: `--provider newsapi` (needs `NEWSAPI_KEY`) or `--provider gdelt` (no key) to correlate with bad-day spikes.
- `scripts/emit_news_windows.py` auto-detects bad windows from a trading log (p_bad/bad_flag) and fetches news/events per window (defaults to GDELT, no API key). Files land under `logs/news_events/` with per-window CSVs plus a summary.
