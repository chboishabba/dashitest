...
- [x] (EXEC) Add per-asset adaptive quantile score-threshold mode (`shadow_score_threshold_mode=adaptive_quantile`, `shadow_target_action_rate`, history controls) on top of smooth entropy attenuation.
- [x] (ANALYSIS) Run 3-point adaptive-target sweep (target 10%) at 0.05/0.075/0.10 (`logs/shadow/shadow_signal_report_20260313T162823Z_adapt10_005.md`, `...163325Z_adapt10_075.md`, `...163836Z_adapt10_100.md`): SPY stabilizes near target (~10–11%), BTC remains high (~18–29%) on the short tape due threshold-warmup/history sensitivity.
- [x] (EXEC) Add offline/prefit adaptive-quantile initialization from prior `trading_log*.csv` score distributions (per-asset seeding of `shadow_score_adjusted`) with CLI controls: `--shadow-score-threshold-prefit`, `--shadow-score-threshold-prefit-max-values`.
- [x] (EXEC) Fix prefit CSV ingestion on large log rows by raising `csv.field_size_limit` in `engine.loop._load_prefit_shadow_scores` (resolved `_csv.Error: field larger than field limit`).
- [x] (EXEC) Add strict prefit seed scoping (`--shadow-score-threshold-prefit-family`) so adaptive initialization only reads same run-family logs (same symbol + matching family substring), reducing cross-regime threshold drag.
- [x] (ANALYSIS) Re-run adaptive prefit with family-scoped seeding (`--shadow-score-threshold-prefit-family prefit_005_kdirshadow_seed200`) and compare action-rate/ACT-vs-HOLD deltas versus unscoped prefit (`logs/shadow/shadow_signal_report_20260315T081325Z_prefit_family_compare.md`): family-scoping works technically but does not materially change action-rate or ACT/HOLD outcomes vs the `seed200` run.
