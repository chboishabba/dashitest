...
- **Task B+ instrumentation + tactical recap**
  - Keep the semantic tile logging/local mean probe as a permanent sanity check so Stage A’s learner → Vulkan causality stays visible (see `CONTEXT.md:32224-32450` for the explanation that global means stay flat by design).
  - Build the next experiment branch now that instrumentation is stable: add non-degenerate regimes (`--gate-density-bins` or a multi-level `active_tile_ratio` regime, and/or the alternating control with richer payloads) so Stage B evaluates genuine variation instead of nearly-constant labels.
  - Decide which path to follow from `CONTEXT.md:32528-32720` (research: swap payload / second axis / noise; productization: observer module + docs + CLI; or intentional pause) and write a targeted follow-up note in TODO to capture that choice before continuing.
  - **Alternating control (power test)**: run `--regime-mode alternating --observer-class corr --epochs 64 --plan-hit-perms 200` so Stage B exercises a balanced, high-entropy label set and verifies whether the scalar/correlation observer class can detect any spatial footprint (`CONTEXT.md:36915-36955`).
- **Observer-ladder research**: implement B1 (adjacent tile correlations / gradient energy) and optionally B2 (tiny CNN with blocked-permutation training) so Stage B can search for nonlocal structure without giving the observer privileged coordinates (`CONTEXT.md:34493-34729` and `CONTEXT.md:36918-36923`).
- **Pause/productize**: if the blind observer persistently fails even under alternating labels, document the invariance claim, freeze the instrumentation, and treat Task B+ as closed before taking on new research or product work (`CONTEXT.md:32580-32650` and `CONTEXT.md:36923-36940`).
- **Stage B2 acceptance checklist**
...
## Additional TODOs (from dashitest Copy)
